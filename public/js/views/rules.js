...
export function renderRules(mount) {
  mount.append(h('div', { class: 'wrap wrap--narrow stack stack--lg' },
    h('div', {},
      ...
      h('h2', { class: 'h1', style: { fontSize: 'clamp(30px,5vw,48px)', marginTop: '6px' } }, 'ПРАВИЛА'),
      h('p', { class: 'lead', style: { marginTop: '12px', maxWidth: '54ch' } },
        'Две фишки на доске 9 × 9 идут навстречу друг другу. Побеждает тот, кто первым '
    ...
    section('Цель', diagGoal(),
      ['Красная фишка стартует внизу и должна попасть в любую клетку верхнего ряда.',
       'Белая стартует сверху и идёт в нижний ряд.',
    ...
    section('Стены', diagWall(),
      ['У каждого игрока свой запас стен (по умолчанию 10).',
       ...
       'Главное ограничение: после постановки у обоих игроков должен остаться хотя бы один путь до своей цели. Полностью замуровать соперника нельзя.']),

    section('Прыжки', diagJump(),
      ['Если фишки стоят вплотную, можно перепрыгнуть соперника и встать сразу за ним.',
       'Если за соперником стена или край доски, прыжок заменяется на диагональный обход — вправо или влево от него.',
    ...
    panelBlock('Управление', [
      ...
      ctrl('Наведение на паз', 'появляется призрак стены; красный — так поставить нельзя'),
      ctrl('R', 'поворот стены между горизонтальной и вертикальной — работает и без курсора на доске'),
      ctrl('Первое касание', 'на телефоне прицеливает стену, второе — ставит'),
      ctrl('Escape', 'снять прицел'),
    ]),

    panelBlock('Нотация', [
      ctrl('e2', 'ход фишкой на клетку e2 — колонки a…i слева направо, ряды 1…9 снизу вверх'),
...
function wallH(r, c, color) {
  return node('rect', {
    x: c * STEP, y: r * STEP + S, width: 2 * S + G, height: G, rx: 2, fill: color,
  });
}

function wallV(r, c, color) {
  return node('rect', {
    x: c * STEP + S, y: r * STEP, width: G, height: 2 * S + G, rx: 2, fill: color,
  });
}
...
function diagGoal() {
  return svg(
    ...gridCells([
      { r: 0, c: 0, fill: 'rgba(220,38,38,.22)', stroke: 'rgba(220,38,38,.4)' },
      { r: 0, c: 1, fill: 'rgba(220,38,38,.22)', stroke: 'rgba(220,38,38,.4)' },
      { r: 0, c: 2, fill: 'rgba(220,38,38,.22)', stroke: 'rgba(220,38,38,.4)' },
      { r: 0, c: 3, fill: 'rgba(220,38,38,.22)', stroke: 'rgba(220,38,38,.4)' },
      { r: 0, c: 4, fill: 'rgba(220,38,38,.22)', stroke: 'rgba(220,38,38,.4)' },
      { r: 4, c: 0, fill: 'rgba(228,228,231,.14)' },
      { r: 4, c: 1, fill: 'rgba(228,228,231,.14)' },
      { r: 4, c: 2, fill: 'rgba(228,228,231,.14)' },
      { r: 4, c: 3, fill: 'rgba(228,228,231,.14)' },
      { r: 4, c: 4, fill: 'rgba(228,228,231,.14)' },
    ]),
